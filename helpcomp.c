...
static void calc_offsets(struct name *name)
{
	for (; name != NULL; name = name->prev)
	{
		...
		if (name->attr != SCR_NAME)
		{
			...
			if (name->attr == ATTR_SENSITIVE)
				offset->scr_code |= 0x8000;
			name_cnt++;
			strcpy(name_array + str_offset, name->name);
			str_offset += strlen(name->name) + 1;
			/* FIXME: too late for check */
			if (name_cnt >= MAX_SCREENS || str_offset >= MAX_TOTALNAMES)
...
static void read_commandfile(const char *path)
{
	...
	while (c != EOF)
	{
		while (isspace(c)) /* FIXME: maybe called with c == EOF */
			c = hc_getc();
		if (c == ';')
		{
			c = hc_getc();
			...
			while (isalnum(c))
			{
				...
				case 'T':
					...
					if (isdigit(c))
						...
						fatal("Digit expected after '='\n"); /* FIXME use message from main */
					break;
				default:
