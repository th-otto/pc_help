...
static char const screen_title_de[] = "Index der verf\201gbaren Schl\201sselw\224rter:";
static char const copyright_name_de[] = "screen(\"Copyright\")";
static char no_index_entry_de[] = "screen(\"a..\")" "Zu diesem Buchstaben ist kein Eintrag vorhanden." "\\end" CTRL_Z_S;
static char const index_name_other_de[] = "Sonstiges";

static char const screen_title_en[] = "Index of available keywords:";
static char const copyright_name_en[] = "screen(\"Version\")\r\n";
static char no_index_entry_en[] = "screen(\"a..\")" "No keywords starting with this letter." "\\end" CTRL_Z_S;
static char const index_name_other_en[] = "Other";
...
static char const copyright[] = "\r\ngenerated by hc (c) 1990 Borland International, Inc.\\end" CTRL_Z_S;
...
void generate_index(void)
{
	...
	int screen_no;
	char *no_index_entry;
	const char *index_name_other;

	switch (options.country)
	{
	case COUNTRY_DE:
	case COUNTRY_SG:
		no_index_entry = no_index_entry_de;
		index_name_other = index_name_other_de;
		break;
	default:
		no_index_entry = no_index_entry_en;
		index_name_other = index_name_other_en;
		break;
	}
	...
	{
		if (i == INDEX_CNT - 1)
			strcpy(index_name, index_name_other);
		...
		} else
		{
			if (did_index == FALSE)
			{
				...
				strcpy((char *)hc_inbuf, no_index_entry);
...
void generate_index_page(void)
{
	...
	case COUNTRY_DE:
	case COUNTRY_SG:
		screen_title = screen_title_de;
		index_name_other = index_name_other_de;
		break;
	default:
		screen_title = screen_title_en;
		index_name_other = index_name_other_en;
		break;
	}
	sprintf((char *)hc_inbuf, index_page, screen_title, index_name_other);
	hc_inbuf_ptr = hc_inbuf;
	parse_file();
}
...
void generate_copyright_page(void)
{
	...
	case COUNTRY_DE:
	case COUNTRY_SG:
		sprintf((char *)hc_inbuf, "%s%s wurde \201bersetzt am %d.%s %d.\r\n%s", copyright_name_de, name, tm.tm_mday, get_month_name(options.country, tm.tm_mon), tm.tm_year + 1900, copyright);
		break;
	default:
		sprintf((char *)hc_inbuf, "%s%s was formatted on %d %s %d.\r\n%s", copyright_name_en, name, tm.tm_mday, get_month_name(options.country, tm.tm_mon), tm.tm_year + 1900, copyright);
		break;
	}
	hc_inbuf_ptr = hc_inbuf;
	parse_file();
